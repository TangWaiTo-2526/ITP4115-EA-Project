...
class EditProfileForm(FlaskForm):
    ...
    username = StringField(_l('姓名'), validators=[DataRequired(message=_l('請填寫姓名'))])
    birthday = DateField(_l('生日'), validators=[Optional()], format='%Y-%m-%d')
    nationality = StringField(_l('國籍'), validators=[Optional(), Length(max=64)])
    mobile_phone = StringField(
        _l('手提電話號碼'),
        validators=[
            Optional(),
            Regexp(r'^\d{8}$', message=_l('請輸入 8 位數字香港手提電話號碼')),
        ],
    ...
    home_phone = StringField(
        ...
        description=_l('送貨地址聯絡用（存於收貨地址）'),
    )
    mail = StringField(
        _l('電郵'),
        validators=[
            DataRequired(message=_l('請填寫電郵')),
            Email(message=_l('請輸入有效的電郵地址')),
        ],
    )
    comm_language = SelectField(
        _l('通訊語言'),
        choices=[('繁體', _l('繁體')), ('English', _l('English'))],
        validators=[Optional()],
    )
    addr_unit = StringField(
        _l('單位'),
        validators=[Optional(), Length(max=64)],
        render_kw={'placeholder': _l('例如：03號／A室')},
    )
    addr_floor = StringField(
        _l('樓層'),
        validators=[Optional(), Length(max=32)],
        render_kw={'placeholder': _l('例如：9樓')},
    )
    ...
    addr_region = SelectField(
        _l('區域'),
        choices=[('', _l('請選擇'))] + [(r, _l(r)) for r in HK_REGIONS],
        validators=[Optional()],
    )
    addr_district = SelectField(_l('地區'), choices=[('', _l('請選擇'))], validators=[Optional()])
    marketing_opt_out = BooleanField(
        _l('我不希望接受有關最新的優惠、促銷、折扣或任何類似的訊息。'),
    )
    brand_fortress = BooleanField(_l('豐澤'), default=True)
    brand_parknshop = BooleanField(_l('百佳'), default=True)
    brand_watsons = BooleanField(_l('屈臣氏'), default=True)
    brand_moneyback = BooleanField(_l('易賞錢合作伙伴'), default=True)
    submit = SubmitField(_l('儲存'), render_kw={'class': 'account-profile-submit-1'})
    ...
    def __init__(self, original_username, original_mail, *args, **kwargs):
        ...
        if reg in HK_DISTRICTS:
            self.addr_district.choices = [('', _l('請選擇'))] + [(d, d) for d in HK_DISTRICTS[reg]]
        else:
            self.addr_district.choices = [('', _l('請選擇'))]
    ...
    def validate_username(self, username):
        ...
        if new_name != (self.original_username or '').strip():
            ...
            if user is not None:
                raise ValidationError(_l('此姓名／使用者名稱已被使用，請換一個。'))
    ...
    def validate_mail(self, mail):
        ...
        if candidate != self.original_mail:
            ...
            if user is not None:
                raise ValidationError(_l('此電郵已被註冊，請換一個。'))
    ...
    def validate_addr_district(self, field):
        ...
        if d and r and d not in HK_DISTRICTS.get(r, ()):
            raise ValidationError(_l('請選擇與區域相符的地區。'))
...
class ProductReviewForm(FlaskForm):
    review_text = TextAreaField(
        _l('評論內容'),
        validators=[
            DataRequired(message=_l('請輸入評論內容')),
            Length(max=1000, message=_l('評論內容不可超過 1000 字')),
        ],
        render_kw={
            'rows': 3,
            'placeholder': _l('請輸入你對這件商品的評價'),
        },
    )
    submit = SubmitField(_l('提交評論'))
